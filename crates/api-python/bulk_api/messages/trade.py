...
def _write_builder_code(builder_code: Optional["BuilderCode"]) -> bytes:
    if builder_code is None:
        return b''
    if builder_code.fee < 1 or builder_code.fee > 15:
        raise ValueError("builder-code fee must be 1..=15 bps")
    return bytes([1]) + _write_pubkey(builder_code.to) + _write_u8(builder_code.fee)
