...
pub struct RandomWsMarketMaker {
    config: RmmConfig,
    client: BulkWsClient,
    ou: OuProcess,
...
impl RandomWsMarketMaker {
    ...
    async fn tick(&mut self) -> eyre::Result<()> {
        ...
        if !prelude {
            ...
            let mut chunks: Vec<(u64, Vec<Action>)> = Vec::new();
            let mut first = true;
            ...
            let futures: Vec<_> = chunks
                .into_iter()
                .map(|(n, actions)| self.client.place_orders(actions, None, Some(n)))
                .collect();
