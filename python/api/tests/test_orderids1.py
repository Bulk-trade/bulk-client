...
class OrderIDTest:
    ...
    async def run_test(self):
        ...
        try:
            ...
            test_nonce = int(time.time_ns() / 1000.0)
            ...
            logger.info(f"  Nonce: {test_nonce}")
            ...
            logger.info("\n[Cleanup] Canceling test order...")
            await asyncio.sleep(1)

            try:
                cancel_response = await self.client.cancel_order(
                    symbol="BTC-USD",
                    order_id=self.returned_order_id,
                    timeout=10.0
                )
                logger.info(f"✓ Order canceled: {cancel_response.status}")
            except Exception as e:
                logger.warning(f"Cancel failed (order may have already been removed): {e}")
