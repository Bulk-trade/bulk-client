...
Executes a ready proposal. The multisig address defaults to:
  ${DEFAULT_MULTISIG}
EOF
}
  ...
  case "$1" in
    ...
    *)
      if [[ -n "${proposal_id}" ]]; then
        echo "error: multisig-exec accepts exactly one proposal ID" >&2
        usage
        exit 2
      fi
      proposal_id="$1"
...
if [[ -z "${proposal_id}" ]]; then
  echo "error: multisig-exec requires a proposal ID" >&2
  usage
  exit 2
fi
...
bulk_admin_run multisig-execute "${multisig}" "${proposal_id}"
