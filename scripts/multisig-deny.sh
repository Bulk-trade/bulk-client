...
Rejects a proposal. The multisig address defaults to:
  ${DEFAULT_MULTISIG}
EOF
}
  ...
  case "$1" in
    ...
    *)
      if [[ -n "${proposal_id}" ]]; then
        echo "error: multisig-deny accepts exactly one proposal ID" >&2
        usage
        exit 2
      fi
      proposal_id="$1"
...
if [[ -z "${proposal_id}" ]]; then
  echo "error: multisig-deny requires a proposal ID" >&2
  usage
  exit 2
fi
...
bulk_admin_run multisig-reject "${multisig}" "${proposal_id}"
