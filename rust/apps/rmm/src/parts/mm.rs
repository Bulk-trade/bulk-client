...
impl RandomMarketMaker {
    ...
    pub async fn initialize(config: RmmConfig, private_key: &str) -> eyre::Result<Self> {
        ...
        info!(
            ...
            symbol, config.bulk_ws_url
        );

        let ws_config = WSConfig {
            url: config.bulk_ws_url.clone(),
    ...
    async fn tick(&mut self) -> eyre::Result<()> {
        ...
        let prelude = self.tick_count <= self.config.priming;
        let mut nonce = nonce_base;

        if prelude || self.tick_count % 50 == 0 {
            let timestamp = std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap()
                .as_nanos() as u64;

            let oracle = OraclePrice::new(timestamp, self.config.coin.clone(), mid);
            self.client
                .update_oracle(vec![oracle], Some(nonce))
                .await?;
